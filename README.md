...
- **Per-project knowledge (RAG)**: Paste text, **upload files** (.md, .gosu, .groovy, .json, **PDF**, …), **index codebase paths** (`reference`, `katalon`, `src/lib`, …), or sync saved outputs via **Project knowledge**. Enable **Use project knowledge (RAG)** in the project bar; chunks are embedded and retrieved per accelerator run. PDFs are text-extracted server-side (max 2 MB; scanned/image-only PDFs need OCR first).
