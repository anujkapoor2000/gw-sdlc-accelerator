...
import { TEST_MIGRATOR_SYSTEM } from '../lib/prompts.js'
...
export default function TestMigrator({ project }) {
  ...
  async function run() {
    ...
    try {
      const prompt = `Target automation framework: ${framework}
Primary product: ${product}

Raw manual test case(s) pasted by the client (may contain one or many):
${input}`
      const text = await callClaude({ system: TEST_MIGRATOR_SYSTEM, prompt, maxTokens: 8000 })
  ...
  const verdicts = useMemo(() => {
    ...
    return [...new Set(result.cases.map((c) => c.verdict))]
  }, [result])

  const cases = useMemo(() => {
    if (!result?.cases) return []
    return verdictFilter === 'all' ? result.cases : result.cases.filter((c) => c.verdict === verdictFilter)
    ...
    <div>
      <header className="page-head">
        ...
        <p className="page-desc">
          ...
          Selenium or Cucumber. Every case is checked for gaps that would block or destabilise automation,
          and the exact test data each script needs is called out before you run it.
        </p>
      </header>
      ...
      <div className="panel">
        ...
        <button className="btn btn-primary" onClick={run} disabled={busy || !input.trim()}>
          {busy ? <><span className="spinner" />Converting to automation…</> : 'Convert to automation'}
        </button>
      </div>
      ...
      {result && (
        <>
          {result.summary && (
            <div className="panel">
              ...
              <div className="stat-row" style={{ display: 'flex', flexWrap: 'wrap', gap: 18, marginTop: 12 }}>
                <Stat value={result.summary.casesAnalysed} label="cases analysed" />
                <Stat value={result.summary.automate} label="automate" tone="green" />
                <Stat value={result.summary.automateWithFixes} label="automate after fixes" tone="amber" />
                <Stat value={result.summary.keepManual} label="keep manual" tone="red" />
                <Stat value={`${result.summary.automationReadiness}%`} label="suite readiness" />
              </div>
          ...
          {verdicts.length > 1 && (
            <div className="chips" style={{ marginBottom: 14 }}>
              <button className={`chip ${verdictFilter === 'all' ? 'on' : ''}`} onClick={() => setVerdictFilter('all')}>
                All ({result.cases.length})
              </button>
              {verdicts.map((v) => (
                <button key={v} className={`chip ${verdictFilter === v ? 'on' : ''}`} onClick={() => setVerdictFilter(v)}>
                  {VERDICT_LABEL[v] || v} ({result.cases.filter((c) => c.verdict === v).length})
                </button>
              ))}
            </div>
          )}
          ...
          {result.crossCuttingGaps?.length > 0 && (
            <div className="panel">
              <h3>Cross-cutting gaps</h3>
              <ul className="plain">{result.crossCuttingGaps.map((g, i) => <li key={i}>{g}</li>)}</ul>
            </div>
          )}
          ...
          {result.recommendations?.length > 0 && (
            <div className="panel">
              ...
              <ol style={{ paddingLeft: 20, fontSize: 14 }}>
                {result.recommendations.map((r, i) => <li key={i} style={{ padding: '2px 0' }}>{r}</li>)}
              </ol>
            </div>
          )}
        </>
